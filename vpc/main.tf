...
resource "aws_subnet" "private_subnet" {
  vpc_id                  = "${aws_vpc.vpc.id}"
  count                   = "${length(data.aws_subnets.private-odoo.ids)}"
  cidr_block              = "${element(data.aws_subnets.private-odoo.cidr_blocks, count.index)}"
  availability_zone       = "${element(local.availability_zones, count.index)}"
...
resource "aws_route_table_association" "public" {
  count          = "${length(data.aws_subnets.public-odoo.ids)}"
